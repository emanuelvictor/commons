...
public class Memetic {
    ...
    public void execute(final String nameInstance) {
        // Tamanho da população. O tamanho da população também define a aleatóriedade
        int[][] population = generateRandomPopulation(2, matrix);
        execution(population);
        imprimir(nameInstance, sort(population, calculateFitness(population, matrix))[0], matrix);
    }
    ...
    private void execution(int[][] population) {
